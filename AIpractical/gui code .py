...
# Programming Step
os.chdir(r"C:\Users\alaa3\Desktop\AIpractical")
...
def new_func():
    def save_file(self):
        fileName = QFileDialog.getOpenFileName(self, tr("Open Image"), "/home/alaa3",
...
def Contra_Harmonic_mean():
    ...
    for r in range(rows):
        for c in range(cols):
            contra_mean[r, c] = np.sum(np.power(pad_img[r:r + ksize, c:c + ksize], q + 1)) / np.sum(
                np.power(pad_img[r:r + ksize, c:c + ksize], q))
...
plant_classes = ['Tomato___Late_blight',
                 'Tomato___healthy',
                 'Grape___healthy',
...
def classification():
    ...
    Label(root, text=plant_classes[y_pred[0]], font=("Comic Sans MS", 15, "bold")).place(x=195, y=435)
    return y_pred
...
# Define Image
bg = PhotoImage(file='final_images555.png')
...
bt3 = Button(root, bg="white",
             fg="red", bd=10, relief="raised", font=("Comic Sans MS", 15, "bold"), image=photoimage1,
...
Classification = Menu(menubar, tearoff=0)
...
menubar.add_cascade(label="Classification", menu=Classification)
...
Classification.add_command(label="Plant Disease Diagnosis", command=classification)
